...
def getReleaseInfo(currentRule, network, currentRuntimestep, resElev, usePrevStepAsEstimate):
    ...
    return qPowerhouse, qNonPH, tempNonPH, qLeakage, tempLeakage
...
def operateForFixedShutters(totalPSFlow, maxPenstockFlow, p1Elev, p2Elev, p3Elev, wqTarget, opElevs, shutterLevelTemps):
    if p1Elev == p2Elev and p2Elev == p3Elev:
        ...
        if t1 - t3 < temperatureDiffThreshold:
            [p1Flow, p2Flow, p3Flow] = splitFlowEvenly(totalPSFlow)
        else:
            if wqTarget < t1:
...
def backRouteWQTarget(network, currentRuntimestep, totalFolsomRelease):
    ...
    multiplier = 17.  # Inflow more important than CSTR assumption because of where inflow enters vertically
...
def setForecastTCDoperation(currentRule, network, currentRuntimestep, wqTarget, resElev):
    ...
    qPowerhouse, qNonPH, tempNonPH, qLeakage, tempLeakage = getReleaseInfo(currentRule, network, currentRuntimestep, resElev, True)
    hasRiverOutletFlow = False
    if qNonPH > lowFlowThreshold:
        hasRiverOutletFlow = True
    ...
    wqTarget = backRouteWQTarget(network, currentRuntimestep, totalFolsomRelease)
    if debugOutput: network.printMessage("Temperature target at Folsom Dam: " + str(wqTarget))
    ...
    if iCurStep == currentRuntimestep.getRunTimeWindow().getNumLookbackSteps() + 1:
        ...
        if curDate == startOpDate:  # raise shutters to highest position
        ...
        if curHour == checkOpHour:
            ...
            # Keep current gate elevations  (subroutine this)
