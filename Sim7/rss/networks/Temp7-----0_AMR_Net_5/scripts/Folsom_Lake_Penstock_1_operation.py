...
def runRuleScript(currentRule, network, currentRuntimestep):
	...
	globalVarName = "P1_active_port_calib"
	
	globVar = network.getGlobalVariable(globalVarName)
	if not globVar:
		raise NameError("Global variable: " + globalVarName + " not found.")
	portElev = globVar.getCurrentValue(currentRuntimestep)
	if not isValidValue(portElev):
		raise ValueError("Global variable: " + globalVarName + " has invalid value " +
		                 str(portElev) + " for time step: " + str(currentRuntimestep.step))
	...
	setTCDoperation(currentRule, flowVal, portElev, resElev)
...
def setTCDoperation(currentRule, flowVal, portElev, resElev):
